...
impl XmasCounter {
    ...
    fn process(&mut self, ch: char) {
        ...
        // Examine for 'XMAS'
        if (self.cache[0] == 'X' && self.cache[1] == 'M' && self.cache[2] == 'A' && self.cache[3] == 'S') || (self.cache[3] == 'X' && self.cache[2] == 'M' && self.cache[1] == 'A' && self.cache[0] == 'S') {
            self.count += 1;
        }
    }
}
...
impl Day for Day04 {
    fn main() -> Result<()> {
        ...
        for y_start in 1..y_len { // Left-Side Skipping 0,0 common start
            let mut y = y_start;
            let mut x = 0;
            while x < x_len && y < y_len {
                counter.process(mat[y][x]);
        ...
        for y_start in 1..y_len { // Right-Side Skipping x_len-1,0 common start
            let mut y = y_start;
            let mut x = x_len - 1;
            while y < y_len {
                counter.process(mat[y][x]);
